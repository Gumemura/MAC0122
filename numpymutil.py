...
def converta_3c(imgs, normaliza=False):
    ...
    '''
    saida = []
    # w, h = imgs[0].array.shape
    h, w = imgs[0].array.shape
    if normaliza:
        vmax = max( [img.array.max() for img in imgs] )

    for img in imgs:
        if normaliza:
            img = 255 * (img.array / vmax)
        ret = np.empty((w, h, 3), dtype=np.uint8)
        ret[:, :, 2] = ret[:, :, 1] = ret[:, :, 0] = np.rot90(img.array,3)
        saida.append( ret )
    return saida
...
def converta_1c( numpymagens ):
    ...
    for img in numpymagens:
        npimgs.append( [plt.imshow( img.array, cmap='gray', vmin=0, vmax=255, animated=True)] )
    return npimgs
